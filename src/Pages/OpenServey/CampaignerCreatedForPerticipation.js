...
import { CheckCircle, ArrowLeft, ShareFill } from 'react-bootstrap-icons';
...
const CampaignerCreatedSurveyPage = () => {
  const { id, status } = useParams();
  console.log('seuey:', id);
  console.log('status:', status);
  const { user } = useContext(AuthContext);
  const isCampaigner = user?.isCampaigner || false;
  ...
  useEffect(() => {
    const fetchSurvey = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/surveys/by-user/${id}`);
                ...
                <div className="d-flex align-items-center mt-2">
                  ...
                  <small className="text-muted">
                    {survey.status === 'active' ?
                      `Open until ${new Date(survey.endDate).toLocaleDateString()}` :
                      'Survey closed'}
                  </small>
                </div>
              </div>
...
export default CampaignerCreatedSurveyPage;